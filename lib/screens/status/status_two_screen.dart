...
class StatusTwoScreen extends StatefulWidget {
  const StatusTwoScreen({super.key});

  @override
  State<StatusTwoScreen> createState() => _StatusTwoScreenState();
}

class _StatusTwoScreenState extends State<StatusTwoScreen> {
  ...
  Future<void> _loadData({bool isRefresh = false}) async {
    ...
    try {
      ...
      // STATUS 2: ĐÃ CÓ TÀI XẾ NHẬN
      final response = await ApiService.getAcceptedRides(
        accessToken: token,
        page: _currentPage,
        pageSize: 15,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Đã có tài xế nhận"),
      ...
      body: RefreshIndicator(
        ...
        child: _isLoading && _allRides.isEmpty
            ...
            : _allRides.isEmpty
            ? const Center(
            child: Text("Chưa có chuyến nào được nhận"))
            : ListView.builder(
